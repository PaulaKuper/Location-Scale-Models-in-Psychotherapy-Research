...
#                     Sensitivity Analyses (after revision)                 #
#                                                                           #
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
...
# 3 Influential case sensitivity analysis ------------------------------------
...
res_influentials2 <- rma(yi=.g, sei=.g_se, data= data, method= "REML", test="knha")
res_influentials2 <- influence(res_influentials2)
res_influentials2$is.infl %>% table()

#save(res_influentials2, file= "results/res_sensitivity_influentials_metafor.rda")
#load("results/res_sensitivity_influentials_metafor.rda")

# inspect
data %>% 
  filter(row_number() %in% which(res_influentials2$is.infl)) %>% 
  select(comparison_id, .g, .g_se)
...
res_influentials <- rma(yi=.g, sei=.g_se, data= data_influentials, method= "REML", test="knha")
summary(res_influentials)
confint(res_influentials, type= "PL", digits= 2)

# obtain prediction interval
predict(res_influentials,  digits= 3) 


# create a reduced dataset for the moderation analysis of "format" without 
# studies with aggregated values
data_format <- subset(data_influentials, !(comparison_id %in% comparisons_aggregate_format))

# drop studies with parent-reported outcomes as n= 2
data_rating <- subset(data_influentials, data_influentials$rating !='parent_report')
data_rating$rating <- data_rating$rating %>% droplevels()


# run univariate moderation models
res_mod_sens_influentials <- tibble(
  model_name= c(
    # Intervention and control characteristics
    "condition_arm1", "condition_arm2", "n_sessions_arm1", "format",
  ...
  database = list(data_influentials, data_influentials, data_influentials, data_format, 
                  data_influentials, data_rating, data_influentials, 
                  data_influentials, data_influentials, data_influentials, data_influentials, 
                  data_influentials, data_influentials, data_influentials, data_influentials, 
                  data_influentials),
  model= pmap(list(moderators, model_name, database), fitModels),
...
# inspect results
res_mod_sens_influentials$model

# inspect missingness for each predictor
res_mod_sens_influentials$n_missing  
map(res_mod_sens_influentials$n_missing, ~./nrow(data_influentials))# percent missingness

# name models
names(res_mod_sens_influentials$model) <- res_mod_sens_influentials$model_name

# save
#save(res_mod_sens_influentials, file= "results/res_mod_sens_influentials.rda")
...
reduced_model <- rma(yi=.g, sei=.g_se, scale= ~rob, data=data_influentials, method= "REML",
                     test="knha", control= list(optimizer="BFGS"))
...
reduced_model_session <- data_influentials %>% 
  select(.g, .g_se, n_sessions_arm1, rob) %>% drop_na() %>% 
  rma(yi=.g, sei=.g_se, scale= ~rob, data=., method= "REML", test="knha", 
      control= list(optimizer="BFGS"))

reduced_model_baselineN <- data_influentials %>% 
  select(.g, .g_se, totaln_bl10, rob) %>% drop_na() %>% 
  rma(yi=.g, sei=.g_se, scale= ~rob, data=., method= "REML", test="knha", 
      control= list(optimizer="BFGS"))

reduced_model_percentWomen <- data_influentials %>% 
  select(.g, .g_se, percent_women, rob) %>% drop_na() %>% 
  rma(yi=.g, sei=.g_se, scale= ~rob, data=., method= "REML", test="knha", 
      control= list(optimizer="BFGS"))

reduced_model_rob <- rma(yi=.g, sei=.g_se, scale= ~1, data=data_influentials, method= "REML", 
                         test="knha", control= list(optimizer="BFGS"))

# likelihood ratio tests (separately for format, rating and predictors with missing values)
res_likelihoodRatio <- list(
  n_sessions_arm1 = anova(res_mod_sens_influentials$model[c("n_sessions_arm1")][[1]], reduced_model_session),
  format = anova(res_mod_sens_influentials$model[c("format")][[1]], reduced_model_format),
  rating = anova(res_mod_sens_influentials$model[c("rating")][[1]], reduced_model_rating),
  totaln_bl10 = anova(res_mod_sens_influentials$model[c("totaln_bl10")][[1]], reduced_model_baselineN),
  percent_women = anova(res_mod_sens_influentials$model[c("percent_women")][[1]], reduced_model_percentWomen),
  rob = anova(res_mod_sens_influentials$model[c("rob")][[1]], reduced_model_rob),
  map(res_mod_sens_influentials$model[c( "condition_arm1", "condition_arm2", "country",  
                                     "year100", "target_group", "instrument_red",  "age_group", 
                                     "comorbid_mental", "diagnosis",  "recruitment")], 
      ~anova(.x,reduced_model))
) 

# order the results as in res_mod_sens_influentials and save in results list
res_mod_sens_influentials$res_likelihoodRatio <- c(res_likelihoodRatio[-length(res_likelihoodRatio)], 
                                               res_likelihoodRatio[[7]])%>% 
  .[match(res_mod_sens_influentials$model_name, names(.))]


res_mod_sens_influentials$res_likelihoodRatio


## 1.3 profile likelihood CIs ----------------------------------------------


res_mod_sens_influentials$profileLikelihoodCI <- map(res_mod_sens_influentials$model_name, 
                                                 ~confint(res_mod_sens_influentials$model[c(.x)][[1]], digits= 3))

profileLikelihoodCI = res_mod_sens_influentials$profileLikelihoodCI

save(profileLikelihoodCI, file= "results/res_profile_likelihood_influentials.rda")
#load("results/res_profile_likelihood_influentials.rda")
...
newScales <- function(moderator) {
  levels_count <- length(levels(moderator))
  newscales <- diag(levels_count)
  newscales[, levels_count] <- 4 # set last cells to 4 (RoB =4= low risk studies)
  newscales_list <- split(newscales, row(newscales))
  newscales_list_reordered <- c(newscales_list[length(newscales_list)], newscales_list[-length(newscales_list)])
  newscales_list_reordered <-c(newscales_list_reordered, newscales_list_reordered[1])
  return(newscales_list_reordered)
...
# iterate over newscales_list and compute predictions (all for studies with low RoB)
res_mod_sens_influentials$pred_tau2 <- list(
  condition_arm1= map(newScales(data_influentials$condition_arm1), 
                      ~predict(res_mod_sens_influentials$model[c("condition_arm1")][[1]], 
                               newscale = .x, digits = 3, transf = exp)),
  condition_arm2= map(newScales(data_influentials$condition_arm2), 
                      ~predict(res_mod_sens_influentials$model[c("condition_arm2")][[1]], 
                               newscale = .x, digits = 3, transf = exp)),
  n_sessions_arm1= list(predict(res_mod_sens_influentials$model[c("n_sessions_arm1")][[1]], 
                                newscale= c(0,0), digits = 3, transf = exp),
                        predict(res_mod_sens_influentials$model[c("n_sessions_arm1")][[1]], 
                                newscale= c(mean(data_influentials$n_sessions_arm1,  
                                                 na.rm=TRUE),4), digits = 3, 
                                transf = exp),
                        predict(res_mod_sens_influentials$model[c("n_sessions_arm1")][[1]], 
                                newscale= c(0,4), digits = 3, transf = exp)), 
  format= map(newScales(data_format$format), 
              ~predict(res_mod_sens_influentials$model[c("format")][[1]], 
                       newscale = .x, digits = 3, transf = exp)),
  country= map(newScales(data_influentials$country), 
               ~predict(res_mod_sens_influentials$model[c("country")][[1]], 
                        newscale = .x, digits = 3, transf = exp)),
  rating= map(newScales(data_rating$rating), 
              ~predict(res_mod_sens_influentials$model[c("rating")][[1]], 
                       newscale = .x, digits = 3, transf = exp)),
  rob=list(predict(res_mod_sens_influentials$model[c("rob")][[1]], 
                   newscale= c(0), digits = 3, transf = exp),
           predict(res_mod_sens_influentials$model[c("rob")][[1]], 
                   newscale= c(mean(data_influentials$rob, na.rm=TRUE)), digits = 3, 
                   transf = exp)),
  year100= list(predict(res_mod_sens_influentials$model[c("year100")][[1]], 
                        newscale= c(0,0), digits = 3, transf = exp),
                predict(res_mod_sens_influentials$model[c("year100")][[1]], 
                        newscale= c(mean(data_influentials$year100, na.rm=TRUE),4), digits = 3, 
                        transf = exp),
                predict(res_mod_sens_influentials$model[c("year100")][[1]], 
                        newscale= c(0,4), digits = 3, transf = exp)),
  totaln_bl10= list(predict(res_mod_sens_influentials$model[c("totaln_bl10")][[1]], 
                            newscale= c(0,0), digits = 3, transf = exp),
                    predict(res_mod_sens_influentials$model[c("totaln_bl10")][[1]], 
                            newscale= c(mean(data_influentials$totaln_bl10,  na.rm=TRUE),4), 
                            digits = 3, transf = exp),
                    predict(res_mod_sens_influentials$model[c("totaln_bl10")][[1]], 
                            newscale= c(0,4), digits = 3, transf = exp)),
  target_group= map(newScales(data_influentials$target_group), 
                    ~predict(res_mod_sens_influentials$model[c("target_group")][[1]],
                             newscale = .x, digits = 3, transf = exp)),
  instrument_red= map(newScales(data_influentials$instrument_red),
                      ~predict(res_mod_sens_influentials$model[c("instrument_red")][[1]], 
                               newscale = .x, digits = 3, transf = exp)),
  age_group= map(newScales(data_influentials$age_group), 
                 ~predict(res_mod_sens_influentials$model[c("age_group")][[1]], 
                          newscale = .x, digits = 3, transf = exp)),
  comorbid_mental= map(newScales(data_influentials$comorbid_mental),
                       ~predict(res_mod_sens_influentials$model[c("comorbid_mental")][[1]],
                                newscale = .x, digits = 3, transf = exp)),
  diagnosis= map(newScales(data_influentials$diagnosis), 
                 ~predict(res_mod_sens_influentials$model[c("diagnosis")][[1]], 
                          newscale = .x, digits = 3, transf = exp)),
  percent_women= list(predict(res_mod_sens_influentials$model[c("percent_women")][[1]], 
                              newscale= c(0,0), digits = 3, transf = exp),
                      predict(res_mod_sens_influentials$model[c("percent_women")][[1]], 
                              newscale= c(mean(data_influentials$percent_women, na.rm=TRUE),4), 
                              digits = 3, transf = exp),
                      predict(res_mod_sens_influentials$model[c("percent_women")][[1]], 
                              newscale= c(0,4), digits = 3, transf = exp)),
  recruitment= map(newScales(data_influentials$recruitment), 
                   ~predict(res_mod_sens_influentials$model[c("recruitment")][[1]],
                            newscale = .x, digits = 3, transf = exp))
)
...
# location estimates + omnibus test
tibble(
  moderator = res_mod_sens_influentials$model_name,
  N = map_dbl(res_mod_sens_influentials$model, "k"),
  loc_coef = map_dbl(res_mod_sens_influentials$model, "b")%>% round(3),
  loc_coef_CI_lower = map_dbl(res_mod_sens_influentials$model, "ci.lb")%>% round(3),
  loc_coef_CI_upper = map_dbl(res_mod_sens_influentials$model, "ci.ub")%>% round(3),
  pval_omnibus_test_scale_coefs = map_dbl(res_mod_sens_influentials$model, "QSp")%>% round(3),
  pval_likelihoodRatio = unlist(map(res_mod_sens_influentials$res_likelihoodRatio, ~round(.x$pval, digits = 3)))
)-> res_sensitivity_influentials_location
...
# scale estimates
tibble(
  moderator = rep(res_mod_sens_influentials$model_name,
                  times= map(res_mod_sens_influentials$model, "q") %>% unlist),
  coefficients = unlist(map(res_mod_sens_influentials$model, ~dimnames(.x$Z)[[2]])),
  scale_coef = unlist(map(res_mod_sens_influentials$model, "alpha"))%>% round(3),
  scale_coef_CI_lower= unlist(map(res_mod_sens_influentials$model, "ci.lb.alpha"))%>% round(3),
  scale_coef_CI_lower_profileLikelihod = map(res_mod_sens_influentials$profileLikelihoodCI,
                                             ~map_chr(.x, ~unlist(.x)["random2"]) %>% 
                                               .[-length(.)] %>% as.numeric()) %>% 
    unlist() %>% round(3),
  scale_coef_CI_upper= unlist(map(res_mod_sens_influentials$model, "ci.ub.alpha"))%>% round(3),
  scale_coef_CI_upper_profileLikelihod = map(res_mod_sens_influentials$profileLikelihoodCI,
                                             ~map_chr(.x, ~unlist(.x)["random3"]) %>% 
                                               .[-length(.)] %>% as.numeric()) %>% 
    unlist() %>% round(3),
  scale_coef_se = unlist(map(res_mod_sens_influentials$model, "se.alpha"))%>% round(3),
  tau2 = res_mod_sens_influentials$pred_tau2 %>% unlist %>% .[grep("pred", names(.))] %>% as.numeric() %>% na.omit %>% round(3),
  tau2_CI_lower =res_mod_sens_influentials$pred_tau2 %>% unlist %>% .[grep("ci.lb", names(.))] %>% 
    as.numeric() %>% na.omit %>% round(3),
  tau2_CI_upper = res_mod_sens_influentials$pred_tau2 %>% unlist %>% .[grep("ci.ub", names(.))] %>% 
    as.numeric() %>% na.omit %>% round(3),
  F_t_z_val = unlist(map(res_mod_sens_influentials$model, "zval.alpha"))%>% round(3),
  pval = unlist(map(res_mod_sens_influentials$model, "pval.alpha")) %>% round(3)
) -> res_sensitivity_influentials_scale

res_sensitivity_influentials_scale

# save in excel sheet
write.xlsx(res_sensitivity_influentials_location, file = "results/results_influentials.xlsx",
           sheetName = "Inflentials sensitivity analysis (location +omnibus)", append = TRUE)

# add a second data set in a new worksheet
write.xlsx( res_sensitivity_influentials_scale, file = "results/results_influentials.xlsx", 
           sheetName="Influentials sensitivity analysis (scale)", append=TRUE)

# save as rda file
save(res_sensitivity_influentials_location, file="results/res_sensitivity_influentials_location.rda")
save( res_sensitivity_influentials_scale, file="results/ res_sensitivity_influentials_scale.rda")
...
data[c(which(!data$comparison_id %in% data_outliers$comparison_id)),"comparison_id"]

unique(data_outliers$study) %>% length()

res_outliers <- rma(yi=.g, sei=.g_se, data= data_outliers, method= "REML", test="knha")
summary(res_outliers)
confint(res_outliers, type= "PL", digits= 2)

# obtain prediction interval
predict(res_outliers,  digits= 3) 
...
# create a reduced dataset for the moderation analysis of "format" without 
# studies with aggregated values
data_format <- subset(data_outliers, !(comparison_id %in% comparisons_aggregate_format))

# drop studies with parent-reported outcomes as n= 2
data_rating <- subset(data_outliers, data_outliers$rating !='parent_report')
data_rating$rating <- data_rating$rating %>% droplevels()


# run univariate moderation models
res_mod_sens_outliers <- tibble(
  model_name= c(
    # Intervention and control characteristics
    "condition_arm1", "condition_arm2", "n_sessions_arm1", "format",
  ...
  database = list(data_outliers, data_outliers, data_outliers, data_format, 
                           data_outliers, data_rating, data_outliers, data_outliers, 
                           data_outliers, data_outliers, data_outliers, data_outliers, 
                           data_outliers, data_outliers, data_outliers, data_outliers),
  model= pmap(list(moderators, model_name, database), fitModels),
...
# inspect results
res_mod_sens_outliers$model$recruitment



# inspect missingness for eainstrument_red# inspect missingness for each predictor
res_mod_sens_outliers$n_missing  
map(res_mod_sens_outliers$n_missing, ~./nrow(data_outliers))# percent missingness

# name models
names(res_mod_sens_outliers$model) <- res_mod_sens_outliers$model_name

# save
save(res_mod_sens_outliers, file= "results/res_mod_sens_outliers.rda")
load("results/res_mod_sens_outliers.rda")
...
reduced_model <- rma(yi=.g, sei=.g_se, scale= ~rob, data=data_outliers, method= "REML",
                     test="knha", control= list(optimizer="BFGS"))
...
reduced_model_session <- data_outliers %>% 
  select(.g, .g_se, n_sessions_arm1, rob) %>% drop_na() %>% 
  rma(yi=.g, sei=.g_se, scale= ~rob, data=., method= "REML", test="knha", 
      control= list(optimizer="BFGS"))

reduced_model_baselineN <- data_outliers %>% 
  select(.g, .g_se, totaln_bl10, rob) %>% drop_na() %>% 
  rma(yi=.g, sei=.g_se, scale= ~rob, data=., method= "REML", test="knha", 
      control= list(optimizer="BFGS"))

reduced_model_percentWomen <- data_outliers %>% 
  select(.g, .g_se, percent_women, rob) %>% drop_na() %>% 
  rma(yi=.g, sei=.g_se, scale= ~rob, data=., method= "REML", test="knha", 
      control= list(optimizer="BFGS"))

reduced_model_rob <- rma(yi=.g, sei=.g_se, scale= ~1, data=data_outliers, method= "REML", 
                         test="knha", control= list(optimizer="BFGS"))

# likelihood ratio tests (separately for format, rating and predictors with missing values)
res_likelihoodRatio <- list(
  n_sessions_arm1 = anova(res_mod_sens_outliers$model[c("n_sessions_arm1")][[1]], reduced_model_session),
  format = anova(res_mod_sens_outliers$model[c("format")][[1]], reduced_model_format),
  rating = anova(res_mod_sens_outliers$model[c("rating")][[1]], reduced_model_rating),
  totaln_bl10 = anova(res_mod_sens_outliers$model[c("totaln_bl10")][[1]], reduced_model_baselineN),
  percent_women = anova(res_mod_sens_outliers$model[c("percent_women")][[1]], reduced_model_percentWomen),
  rob = anova(res_mod_sens_outliers$model[c("rob")][[1]], reduced_model_rob),
  map(res_mod_sens_outliers$model[c( "condition_arm1", "condition_arm2", "country",  
                       "year100", "target_group", "instrument_red",  "age_group", 
                       "comorbid_mental", "diagnosis",  "recruitment")], 
      ~anova(.x,reduced_model))
) 

# order the results as in res_mod_sens_outliers and save in results list
res_mod_sens_outliers$res_likelihoodRatio <- c(res_likelihoodRatio[-length(res_likelihoodRatio)], 
                                 res_likelihoodRatio[[7]])%>% 
  .[match(res_mod_sens_outliers$model_name, names(.))]


res_mod_sens_outliers$res_likelihoodRatio


## 1.3 profile likelihood CIs ----------------------------------------------

# for all non-fixed scale parameters
res_mod_sens_outliers$profileLikelihoodCI <- map(res_mod_sens_outliers$model_name, 
                                   ~confint(res_mod_sens_outliers$model[c(.x)][[1]], digits= 3))

profileLikelihoodCI = res_mod_sens_outliers$profileLikelihoodCI

save(profileLikelihoodCI, file= "results/res_profile_likelihood_outliers.rda")
load("results/res_profile_likelihood_outliers.rda")
...
# iterate over newscales_list and compute predictions (all for studies with low RoB)
res_mod_sens_outliers$pred_tau2 <- list(
  condition_arm1= map(newScales(data_outliers$condition_arm1), 
                      ~predict(res_mod_sens_outliers$model[c("condition_arm1")][[1]], 
                               newscale = .x, digits = 3, transf = exp)),
  condition_arm2= map(newScales(data_outliers$condition_arm2), 
                      ~predict(res_mod_sens_outliers$model[c("condition_arm2")][[1]], 
                               newscale = .x, digits = 3, transf = exp)),
  n_sessions_arm1= list(predict(res_mod_sens_outliers$model[c("n_sessions_arm1")][[1]], 
                                newscale= c(0,0), digits = 3, transf = exp),
                        predict(res_mod_sens_outliers$model[c("n_sessions_arm1")][[1]], 
                                newscale= c(mean(data_outliers$n_sessions_arm1,  
                                                 na.rm=TRUE),4), digits = 3, 
                                transf = exp),
                        predict(res_mod_sens_outliers$model[c("n_sessions_arm1")][[1]], 
                                newscale= c(0,4), digits = 3, transf = exp)), 
  format= map(newScales(data_format$format), 
              ~predict(res_mod_sens_outliers$model[c("format")][[1]], 
                       newscale = .x, digits = 3, transf = exp)),
  country= map(newScales(data_outliers$country), 
               ~predict(res_mod_sens_outliers$model[c("country")][[1]], 
                        newscale = .x, digits = 3, transf = exp)),
  rating= map(newScales(data_rating$rating), 
              ~predict(res_mod_sens_outliers$model[c("rating")][[1]], 
                       newscale = .x, digits = 3, transf = exp)),
  rob=list(predict(res_mod_sens_outliers$model[c("rob")][[1]], 
                   newscale= c(0), digits = 3, transf = exp),
           predict(res_mod_sens_outliers$model[c("rob")][[1]], 
                   newscale= c(mean(data_outliers$rob, na.rm=TRUE)), digits = 3, 
                   transf = exp)),
  year100= list(predict(res_mod_sens_outliers$model[c("year100")][[1]], 
                        newscale= c(0,0), digits = 3, transf = exp),
                predict(res_mod_sens_outliers$model[c("year100")][[1]], 
                        newscale= c(mean(data_outliers$year100, na.rm=TRUE),4), digits = 3, 
                        transf = exp),
                predict(res_mod_sens_outliers$model[c("year100")][[1]], 
                        newscale= c(0,4), digits = 3, transf = exp)),
  totaln_bl10= list(predict(res_mod_sens_outliers$model[c("totaln_bl10")][[1]], 
                            newscale= c(0,0), digits = 3, transf = exp),
                    predict(res_mod_sens_outliers$model[c("totaln_bl10")][[1]], 
                            newscale= c(mean(data_outliers$totaln_bl10,  na.rm=TRUE),4), 
                            digits = 3, transf = exp),
                    predict(res_mod_sens_outliers$model[c("totaln_bl10")][[1]], 
                            newscale= c(0,4), digits = 3, transf = exp)),
  target_group= map(newScales(data_outliers$target_group), 
                    ~predict(res_mod_sens_outliers$model[c("target_group")][[1]],
                             newscale = .x, digits = 3, transf = exp)),
  instrument_red= map(newScales(data_outliers$instrument_red),
                      ~predict(res_mod_sens_outliers$model[c("instrument_red")][[1]], 
                               newscale = .x, digits = 3, transf = exp)),
  age_group= map(newScales(data_outliers$age_group), 
                 ~predict(res_mod_sens_outliers$model[c("age_group")][[1]], 
                          newscale = .x, digits = 3, transf = exp)),
  comorbid_mental= map(newScales(data_outliers$comorbid_mental),
                       ~predict(res_mod_sens_outliers$model[c("comorbid_mental")][[1]],
                                newscale = .x, digits = 3, transf = exp)),
  diagnosis= map(newScales(data_outliers$diagnosis), 
                 ~predict(res_mod_sens_outliers$model[c("diagnosis")][[1]], 
                          newscale = .x, digits = 3, transf = exp)),
  percent_women= list(predict(res_mod_sens_outliers$model[c("percent_women")][[1]], 
                              newscale= c(0,0), digits = 3, transf = exp),
                      predict(res_mod_sens_outliers$model[c("percent_women")][[1]], 
                              newscale= c(mean(data_outliers$percent_women, na.rm=TRUE),4), 
                              digits = 3, transf = exp),
                      predict(res_mod_sens_outliers$model[c("percent_women")][[1]], 
                              newscale= c(0,4), digits = 3, transf = exp)),
  recruitment= map(newScales(data_outliers$recruitment), 
                   ~predict(res_mod_sens_outliers$model[c("recruitment")][[1]],
                            newscale = .x, digits = 3, transf = exp))
)
...
# location estimates + omnibus test
tibble(
  moderator = res_mod_sens_outliers$model_name,
  N = map_dbl(res_mod_sens_outliers$model, "k"),
  loc_coef = map_dbl(res_mod_sens_outliers$model, "b")%>% round(3),
  loc_coef_CI_lower = map_dbl(res_mod_sens_outliers$model, "ci.lb")%>% round(3),
  loc_coef_CI_upper = map_dbl(res_mod_sens_outliers$model, "ci.ub")%>% round(3),
  pval_omnibus_test_scale_coefs = map_dbl(res_mod_sens_outliers$model, "QSp")%>% round(3),
  pval_likelihoodRatio = unlist(map(res_mod_sens_outliers$res_likelihoodRatio, ~round(.x$pval, digits = 3)))
)-> res_sens_outliers
...
# scale estimates
tibble(
  moderator = rep(res_mod_sens_outliers$model_name,
                  times= map(res_mod_sens_outliers$model, "q") %>% unlist),
  coefficients = unlist(map(res_mod_sens_outliers$model, ~dimnames(.x$Z)[[2]])),
  scale_coef = unlist(map(res_mod_sens_outliers$model, "alpha"))%>% round(3),
  scale_coef_CI_lower= unlist(map(res_mod_sens_outliers$model, "ci.lb.alpha"))%>% round(3),
  scale_coef_CI_lower_profileLikelihod = map(res_mod_sens_outliers$profileLikelihoodCI,
                                             ~map_chr(.x, ~unlist(.x)["random2"]) %>% 
                                               .[-length(.)] %>% as.numeric()) %>% 
    unlist() %>% round(3),
  scale_coef_CI_upper= unlist(map(res_mod_sens_outliers$model, "ci.ub.alpha"))%>% round(3),
  scale_coef_CI_upper_profileLikelihod = map(res_mod_sens_outliers$profileLikelihoodCI,
                                             ~map_chr(.x, ~unlist(.x)["random3"]) %>% 
                                               .[-length(.)] %>% as.numeric()) %>% 
    unlist() %>% round(3),
  scale_coef_se = unlist(map(res_mod_sens_outliers$model, "se.alpha"))%>% round(3),
  tau2 =  res_mod_sens_outliers$pred_tau2 %>% unlist %>% .[grep("pred", names(.))] %>% as.numeric() %>% na.omit %>% round(3),
  tau2_CI_lower =  res_mod_sens_outliers$pred_tau2 %>% unlist %>% .[grep("ci.lb", names(.))] %>% 
    as.numeric() %>% na.omit %>% round(3),
  tau2_CI_upper =  res_mod_sens_outliers$pred_tau2 %>% unlist %>% .[grep("ci.ub", names(.))] %>% 
    as.numeric() %>% na.omit %>% round(3),
  F_t_z_val = unlist(map(res_mod_sens_outliers$model, "zval.alpha"))%>% round(3),
  pval = unlist(map(res_mod_sens_outliers$model, "pval.alpha")) %>% round(3)
) -> res_sens_outliers_scale
res_sens_outliers_scale


# save in excel sheet
write.xlsx(res_sens_outliers, file = "results/results_outliers.xlsx",
           sheetName = "Sensitivity analysis outliers (location +omnibus)", append = TRUE)

# save scale results in a new worksheet
write.xlsx(res_sens_outliers_scale, file = "results/results_outliers.xlsx", 
           sheetName="Sensitivity analysis outliers 2 (scale)", append=TRUE)


# save as rda file
save(res_mod_sens_outliers, file="results/res_mod_sens_outliers.rda")
save(res_sens_outliers_scale, file="results/res_mod_sens_outliers_scale.rda")
