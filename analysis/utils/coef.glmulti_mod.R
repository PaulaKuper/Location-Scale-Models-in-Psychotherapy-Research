...
coef.glmulti <- function(res, select="all", varweighting="Buckland", icmethod="Lukacs", 
                         alphaIC=0.05, models, formulas, ...) 
{
  
  # get AICC values
  res_weightable= weightable(res)[, c("model", "aicc")]
  
  # sort weights according to original model formulas (not in descending order)
  formulas_orig= as.character(formulas)
  ww = res_weightable[match(formulas_orig, res_weightable$model),]$aicc
  ...
  # extract the names of the coefficients (namou) by concatenating the 
  ...
  # use the results obtained from the gettou function and assign them to the 
